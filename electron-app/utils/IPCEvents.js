...
class IPCEvents{
  ...
  queryUsersEvent(){
	...
	const db = new sqlite3.Database(path.resolve(__dirname,`../../assets/databases/${this.selectedDatabase}`));

	let sqlQuery = "select * from users"
      
	if(firstName != ""){
	  sqlQuery += sqlQuery.includes("where")?` and firstName like '%${firstName}%'`:` where firstName like '%${firstName}%'`
	}
      
	if(lastName != ""){
	  sqlQuery += sqlQuery.includes("where")?` and lastName like '%${lastName}%'`:` where lastName like '%${lastName}%'`
	}
	...
	sqlQuery += sqlQuery.includes("where")?` and weight between ${weight[0]} and ${weight[1]}`:` where weight between ${weight[0]} and ${weight[1]}`
	sqlQuery += sqlQuery.includes("where")?` and year between ${year[0]} and ${year[1]}`:` where year between ${year[0]} and ${year[1]}`
	sqlQuery += sqlQuery.includes("where")?` and height between ${height[0]} and ${height[1]}`:` where height between ${height[0]} and ${height[1]}`
	sqlQuery += sqlQuery.includes("where")?` and leg_length between ${legLength[0]} and ${legLength[1]}`:` where leg_length between ${legLength[0]} and ${legLength[1]}`
