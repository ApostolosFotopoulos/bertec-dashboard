	...
	createDatabaseEvent() {
		ipcMain.on('CREATE_DATABASE', async (_, d) => {
			try {
				...
				let similarDbNamesLen = databases.filter((d) => d.substr(0, d.lastIndexOf('_')) == database).length;
				...
				db.serialize(function() {
					...
					db.run('create table tags(id integer primary key autoincrement, name text, user_id integer)');
				});
				db.close();
			} catch (e) {
				throw new Error(e);
   ...
   */
	deleteDatabaseEvent() {
		ipcMain.on('DELETE_DATABASE', async (_, d) => {
			try {
				let { database } = d;
				const deleteFile = path.resolve(__dirname, `../../assets/databases/${database}`);
		...
		ipcMain.on('CREATE_USER', (_, d) => {
			...
			db.run(
				`insert into users(firstName, lastName, year, other_info, sex, height, leg_length, weight,hospital_code, surgery_date, injury_date, affected_side, created_at,updated_at)` +
					...
					`'${hospitalCode}','${moment(new Date(surgeryDate)).format('DD - MM - YYYY')}','${moment(
						new Date(injuryDate)
					).format('DD - MM - YYYY')}','${affectedSide}','${moment(new Date()).format(
						'DD - MM - YYYY'
